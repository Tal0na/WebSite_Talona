...
// Ícones de marcas (Brands) - IMPORTANTE: Importar do pacote correto
import {
  faGithub,
  faSpotify,
  faYoutube,
...
const ProjectsPage = () => {
  return (
    <div className="min-h-screen w-full dark:bg-[#121212] bg-white font-indie">
      <Navbar />

      <div className="max-w-7xl mx-auto px-4 py-12">
        <h1 className="text-3xl font-bold text-center dark:text-rose-600 text-rose-600 mb-10">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 text-black dark:text-white font-roboto ">
          ...
          <ProjectCard
            ...
            link="/Links"
            // Adicionei um ícone padrão caso queira, ou pode deixar sem
            icon={faTableColumns}
          />
        </div>
      </div>
  ...
  return (
    <div className="bg-white dark:bg-black rounded-xl shadow-md p-6 flex flex-col items-center justify-center">
      {icon && (
        <FontAwesomeIcon
          icon={icon}
          className="text-lg mb-2 dark:text-rose-600 text-rose-600"
        />
      )}
      ...
      <Link href={link} passHref legacyBehavior>
        <a
          className="mt-4 dark:bg-rose-600 bg-rose-600 text-white dark:text-black px-4 py-2 rounded-full inline-block
          hover:bg-rose-700 dark:hover:bg-rose-600 transition-transform duration-300 hover:scale-105"
        >
          Ver Projeto
        </a>
      </Link>
